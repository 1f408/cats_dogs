...
一般的には、APIを作ろうとすると、CGIやAPIサーバのプログラムの用意が必要になります。やりたいこと自体がコマンドをいくつか実行するぐらい簡単だと、それ以外の部分の構築コストが見合いません。  
そこで、APIに必要な機能のうち、スクリプト自体を除く、以下の機能を代行する仕組みをcats\_dogsに組み込みました。

- スクリプトをAPI経由で実行する仕組み(**cat\_mult\_getexec**が担当)
- APIを呼び出すルール設定の仕組み(**Cat UI**設定ファイル)
- APIの入出力をWeb UIフォームに変換する仕組み(**cat\_tmplview**と**cat\_ui.js**が担当)

これが、**Cat UIプラットフォーム**です。要は、cats\_dogsでは、簡単なスクリプトと設定ファイルで、APIとそのWeb UIが手軽に実現できるわけです。
...
**Cat UIプラットフォーム**は、**cat\_tmplview**、**cat\_ui.js**、**cat\_mult\_getexec**を使った、以下のような構造での連係で、低コストでのWeb UI作成を実現しています。

![Cat UIプラットフォーム](cats_ui.png)

Cat UIでは、具体的には以下のような流れで処理が行なわれます。

1. アクセスされたURLに対応するテンプレートファイルを変換される。(**cat\_tmplview**の処理)
2. `{{cat_ui UI名}}`のテンプレート処理で、指定したUI名のUI設定ファイル(UI名 + `.ui`)にしたがって、Web UIのHTMLを生成する。(**cat\_tmplview**の処理)
...
Cat UIプラットフォームで、Web UIとそのためのAPIを追加するために必要となるファイルは、以下の3種類(設定ファイル2つ、スクリプト1つ)です。

### UI設定ファイル(`*.ui`)

Web UIの入力パラメータとその表示用のラベルを指定するファイルです。
ファイル名は、**UI名**にUI設定ファイルの拡張子(デフォルトは`ui`)を付けたものになります。  
ファイルの書式については、[cat\_tmplview](cat_tmplview.md)のUI設定ファイルの説明を参照してください。

### API設定ファイル(`*.api`)

APIが実行するシェルスクリプトとその引数にわたす値を指定するファイルです。
ファイル名は、**UI名**にAPI設定ファイルの拡張子(デフォルトは`api`)を付けたものになります。  
...
また、上記の**スクリプト作成の支援機能**で説明したプログラム群を使うことで、入力のバリデーションやCSVの出力が簡単に行なえるようになっています。

## 構築フロー

Cat UIプラットフォーム構築の流れは、おおよそ以下のようになります。詳細は各プログラムの仕様を確認してください。

1. Nginxが**cat\_multi\_getexec**を呼び出すようにする。  
    1. **cat\_multi\_getexec**を動作させる。  
    2. API専用のパス(/api等)を決めて、そのパスへNginxの**cat\_multi\_getexec**用のproxy設定を追加する。  
...
- 簡易UI作成を支援するプログラム群
    - [cat\_tmplview](cat_tmplview.md) cat\_uiテンプレート関数を記述するだけでWeb UIを生成できるので、簡易UIの量産が可能です。
    - [cat\_ui.js](./cat_ui_js.md) APIのCSV出力をHTMLのtableに変換するので、出力のテーブル表示も容易です。
- 簡易API作成を支援するプログラム
    - [cat\_multi\_getexec](cat_multi_getexec.md) テキストやCSVを出力するシェルスクリプトと簡易な設定ファイルを用意することで、低コストでAPIが作成できます。
    - [cat\_verify\_type](cat_verify_type.md) シェルスクリプトではチェックしにくいIPアドレスなどをチェックするプログラムです。スクリプトでの入力値のチェックが楽に行えます。
    - [cat\_args2csv](cat_args2csv.md)、[cat\_printcsv](cat_printcsv.md) シェルスクリプトでの、RFC4180のCSVフォーマット生成を支援します。
