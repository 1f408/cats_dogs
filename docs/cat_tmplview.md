...
コマンドラインは、以下の通りです。
```
cat_tmpview [-d <url_path>] 設定ファイル名
```
...
cat\_tmplviewのテンプレートファイルは、Goのtext/tmplateの書式です。  
Goの`text/tmplate`のテンプレート関数として、cats\_dogs共通のテンプレート関数とcat\_tmplview独自のテンプレート関数が追加されています。

cats\_dogs共通のテンプレート関数については、[cats\_dogsテンプレートファイル](tmpl_files.md)の説明を参照してください。  
追加されている、cat\_tmplview独自のテンプレート関数は、以下の通りです。
...
ユーザの権限でのコンテンツの出し分けを行う場合と、[Cat UI](cat_ui.md)を利用する場合は、これらの
cat\_tmplview独自のテンプレート関数を使う必要があります。
